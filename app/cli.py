...
from app.constants.directory import DEFAULT_SUGGEST_LIMIT
...
def all_slugs() -> list[str]:
    ...
    for city in list_cities():
        ...
        slugs.extend(b["слаг"] for b in list_branches(city["слаг"]))
    return slugs


def suggest(slug: str, limit: int = DEFAULT_SUGGEST_LIMIT) -> list[str]:
...
def render_branches(city_slug: str) -> str:
    ...
    """
    branches = list_branches(city_slug)
    if not branches:
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not list_cities():
        print(f"Нет данных в {settings.directory_data_dir} — сначала обнови справочник.")
        return 1
    ...
    print(f"Справочник: {len(list_cities())} городов. {HELP}")
    while True:
        try:
            line = input(PROMPT)
