...
3. По флагу `fetch_metro` тянет станции метро и район с avtoshkoli.ru, где они подписаны
   у каждого филиала.
...
def load_landmarks(landmarks_path: Path | None = None) -> list[dict]:
    """Читает выгрузку ориентиров и типов точек из 2ГИС."""
    path = landmarks_path if landmarks_path is not None else settings.landmarks_path
    if not path.exists():
        return []
...
def run(
    *,
    ...
    Args:
        ...
        fetch_metro_flag: тянуть метро с avtoshkoli.ru.
    ...
    for path in files:
        ...
        metro_map = fetch_metro(slug) if fetch_metro_flag else {}
        if fetch_metro_flag and not metro_map:
