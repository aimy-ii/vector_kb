...
def main() -> int:
    ...
    if not files:
        print("Не найдено data/out/*.json — сначала запусти vektor-scrape")
        return 1
    ...
    for path in files:
        ...
        text = normalize(raw_path.read_text(encoding="utf-8"))
        lines = text.split("\n")
        ...
        fleet, fleet_notes = build_fleet(text, url)
        theory, theory_notes = build_theory(text, url)
