"""Офлайн-тесты пакета справочника: без сети, на встроенных данных."""
...
from app.services.directory_service import city_enum, get_branch, get_city, resolve_city
from app.services.directory_service.lookup import branch_enum
...
def test_public_imports() -> None:
    """Публичные функции импортируются из пакета справочника."""
...
def test_store_reload_updates_memory() -> None:
    """reload() обновляет снимок в памяти и время загрузки."""
    first = directory_store.loaded_at
    count = directory_store.reload()
    assert count > 0
    assert directory_store.loaded_at is not None
    if first is not None:
        assert directory_store.loaded_at >= first
